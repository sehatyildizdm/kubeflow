...
ARGOCD_PASSWORD="${ARGOCD_PASSWORD:--cd5POvPn0HVe5qe}"
...
function argocd_login() {
  ...
  if [[ -n "$_argocd_server_url" ]]; then
    argocd --grpc-web login "$_argocd_server_url" --username "$_argocd_username" --password "$_argocd_password"
  else
    ...
    argocd --grpc-web login --username "$_argocd_username" --password "$_argocd_password"
  fi
  echo_green ">>> DONE"
}
...
function sync_argocd_apps() {
  ...
  exec 3>&1
  set +e
  _cmd_stderr=$(argocd --grpc-web app sync "${_app_names[@]}" "${_sync_args[@]}" 2>&1 >&3 | tee /dev/stderr)
  _cmd_exit_code=$?
  set -e
  exec 3>&-
...
function argocd_app_wait() {
  ...
  argocd --grpc-web app wait "${_app_names[@]}" --health --timeout "$ARGOCD_WAIT_TIMEOUT_SECONDS"
  echo_green ">>> DONE"
}
...
function sync_argocd() {
  ...
  local -a _app_names=()
  read -r -a _app_names <<< "$(argocd --grpc-web app list -l "$_app_selector" -N "$_app_namespace" -o "name" | tr '\n' ' ')"
  ...
  for _app_name in "${_app_names[@]}"; do
    ...
    _app_json=$(argocd --grpc-web app get "$_app_name" -o json --refresh)
